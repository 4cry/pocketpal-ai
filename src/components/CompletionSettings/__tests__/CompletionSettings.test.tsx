...
describe('CompletionSettings', () => {
  ...
  describe('server defaults', () => {
    ...
    it('resets a forwarded sampler to the app default, not the reported one', () => {
      // Storing the app default is what leaves the sampler off the wire; the
      // reported number then applies without being pinned to this session.
      ...
      const {getByTestId} = render(
        <CompletionSettings
          ...
          onChange={onChange}
          serverDefaults={{min_p: wire.min_p}}
        />,
      );
